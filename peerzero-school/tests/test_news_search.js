...
await (async () => {
  ...
  fetchResponses['en.wikipedia.org'] = (url) => {
    if (url.includes('Portal:Current_events')) {
      return {
        ok: true,
        json: async () => ({
          parse: {
            text: {
              '*': '<ul><li>Major earthquake strikes region causing widespread damage and rescue operations begin</li><li>International trade agreement signed between nations</li><li>Short</li><li>' + 'x'.repeat(600) + '</li></ul>',
            },
          },
        }),
      };
...
// ═══════════════════════════════════════════════════════════════════════
// searchCurrentEvents() — pipeline integration
// ═══════════════════════════════════════════════════════════════════════
