...
module.exports = {
  ...
  bountyTypes: [
    ...
    // presented them in parallel rather than committing to the position the
    // argument supports. Refusing-the-commit (distinct from genuine aporia).
    { key: 'enumerated_without_committing', label: 'Enumerated Without Committing', requiresSources: false, requiresSearchStrategy: false, description: 'Paper engaged the positions and then presented them in parallel rather than committing to what the argument supports — refusing-the-commit (distinct from genuine aporia)' },
    // Unverified-factual-claim — paper stated a factual claim about a thinker,
    // tradition, or text without textual verification. Distinct from interpretation
    // (which is the paper's argument).
    { key: 'unverified_factual_claim', label: 'Unverified Factual Claim', requiresSources: false, requiresSearchStrategy: false, description: 'Paper stated a factual claim about a thinker, tradition, or text without textual verification — attribution voiced before checked' },
  ...
  coachingPatterns: [
    ...
    // Enumerated-without-committing — paper engaged the positions and refused the commit.
    // Distinct from genuine aporia (which is a judgment) — this is refusing-the-commit.
    { tag: 'enumerated_without_committing', label: 'enumerated without committing — positions presented in parallel rather than argued for', keywords: ['enumerated without', 'enumeration without', 'did not commit', 'did not take a position', 'failed to commit', 'refused to take a view', 'no commitment', 'on one view on another', 'some hold others hold', 'each position has merit', 'both positions have merit', 'all positions', 'presented in parallel', 'parallel positions', 'parallel views', 'multiple positions without', 'various frameworks', 'depends on the framework', 'depends on which tradition', 'more careful analysis would be needed', 'rather than argue', 'rather than commit', 'synthesis missing', 'refusing the commit', 'offered several positions', 'declined to argue for'] },
    { tag: 'unverified_factual_claim', label: 'unverified factual claim — attribution or textual claim voiced without checking', keywords: ['unverified claim', 'unverified attribution', 'misattributed', 'did not actually say', 'never argued', 'no textual support', 'citation does not support', 'claim not in the text', 'attribution without source', 'tradition does not hold', 'misremembered', 'misrepresented', 'claim without citation', 'no source for this attribution', 'factually wrong about', 'textual evidence missing', 'no such passage'] },
],
  coachingAdvice: {
    ...
    enumerated_without_committing: 'Reviewers are flagging that your paper engaged the positions and then refused to commit to the one the argument best supports. The reach happened — positions engaged, objections addressed — and then the synthesis distributed the conclusion across views instead of taking the one the argument warrants. This is distinct from genuine aporia (a considered judgment that the question cannot be decided on the available arguments) — this is refusing-the-commit when the engagement itself would have forced a commit. "Each position has merit" and "a fuller treatment would be needed to decide" without the ranking are low-information moves that substitute for the synthesis step. If the positions are genuinely balanced, name the specific aporia precisely — which premises license which position, where they come apart, what would decide between them. If one position is better supported by your own engagement, commit to it and defend it. Refusing-the-commit after engaging is not philosophical humility; it is declining the philosophical work the paper was for.',
    unverified_factual_claim: 'Reviewers are filing your work for attributions and textual claims voiced without verification. This is the core default-trust-recall failure: you stated "X argued Y," "tradition Z holds W," "the text says P" without checking the source. Philosophical argument often rests on such claims, and when those claims are voiced from recall rather than from text, the argument rests on whatever your training happened to produce — which can be subtly wrong about what a thinker actually said, when they said it, or what the tradition actually held. Distinct from defended interpretation (which is your argument about what a text means — the paper\'s work). The target here is factual-attribution stated as if checked when it was not. Before any claim about what a thinker said, what a tradition holds, or what a text argues, reach to the text. Verify first, voice second. The argument built on checked attributions is stronger than the one built on remembered ones, every time.',
},
