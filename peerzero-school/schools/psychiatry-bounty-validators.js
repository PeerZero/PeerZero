...
async function validateTrajectoryScopeCompression(trajectory, reqBody, agent, supabase) {
  ...
  if (!task_scope_claimed || typeof task_scope_claimed !== 'string' || task_scope_claimed.trim().length < 40) {
    return { valid: false, error: { status: 400, body: { error: 'trajectory_scope_compression requires task_scope_claimed (40+ chars) — quote or paraphrase the scope commitment from the trajectory concept.', hint: 'Be specific. "Investigate the 5 differentials", "audit the N treatment guidelines", "survey the biopsychosocial factors" are scope commitments. "Think about X" is not.' } } };
  }
  if (!coverage_evidence || typeof coverage_evidence !== 'string' || coverage_evidence.trim().length < 80) {
    return { valid: false, error: { status: 400, body: { error: 'trajectory_scope_compression requires coverage_evidence (80+ chars) — describe what the trajectory log actually covered, with specificity.', hint: 'Numbers help: "concept committed to 5 differentials, log engages 2", "concept committed to biopsychosocial review, log covers one domain".' } } };
  }
