...
module.exports = {
  ...
  bountyTypes: [
    ...
    // Enumerated-without-committing — paper assessed the clinical data and
    // then presented multiple diagnoses or plans in parallel rather than
    // committing to the working formulation. Undifferentiated differential is
    // not humility — it is avoidance of the synthesis step.
    { key: 'enumerated_without_committing',     label: 'Enumerated Without Committing',    requiresSources: false, requiresSearchStrategy: false, description: 'Paper assessed the clinical data and then presented multiple diagnoses or plans in parallel rather than committing to the working formulation — an undifferentiated differential is not humility' },
    // Unverified-factual-claim — paper stated a clinical fact (prevalence, efficacy,
    // mechanism, DSM criterion) without verification. Patient care rests on these
    // being checked, not recalled.
    { key: 'unverified_factual_claim',          label: 'Unverified Factual Claim',         requiresSources: false, requiresSearchStrategy: false, description: 'Paper stated a clinical fact (prevalence, efficacy, mechanism, criterion) without verification against guidelines or evidence' },
  ...
  coachingPatterns: [
    ...
    // Enumerated-without-committing — paper did the workup and refused to commit
    // to the working diagnosis or primary plan.
    { tag: 'enumerated_without_committing', label: 'enumerated without committing — differential listed in parallel rather than prioritized', keywords: ['enumerated without', 'enumeration without', 'did not commit', 'did not prioritize', 'failed to commit', 'undifferentiated differential', 'no working diagnosis', 'no primary plan', 'refused to pick', 'no commitment', 'could be could be', 'each diagnosis possible', 'each plan reasonable', 'all diagnoses equally', 'presented in parallel', 'parallel diagnoses', 'parallel plans', 'multiple diagnoses without', 'multiple plans without', 'depends on further workup', 'more data needed to decide', 'rather than commit', 'working formulation missing', 'did not formulate', 'broad differential without ranking'] },
    { tag: 'unverified_factual_claim', label: 'unverified factual claim — clinical fact stated without verification', keywords: ['unverified claim', 'unverified clinical', 'prevalence unsupported', 'efficacy unsupported', 'criterion not in DSM', 'no guideline cited', 'stat without source', 'no citation for this figure', 'mechanism without citation', 'confidently stated', 'factual claim without source', 'clinical fact unverified', 'no evidence base cited', 'treatment claim unsupported', 'assumed clinical fact', 'did not verify the claim', 'no supporting study', 'claim not in guideline', 'made up a number'] },
],
  coachingAdvice: {
    ...
    enumerated_without_committing: 'Reviewers are flagging that your paper assessed the clinical data and then refused to commit to the working diagnosis or primary plan the data supports. The workup happened — history, exam, investigations — and then the formulation distributed the conclusion across diagnoses or plans instead of producing the working formulation the data warrants. An undifferentiated differential is not clinical humility; it is avoidance of the synthesis step the assessment was for. If the evidence genuinely splits, name the specific split precisely — prior probability, discriminating test, decision point, what would shift the posterior. If one diagnosis or plan is better supported, commit to it as the working formulation (with the differential ranked below) and say why. "Could be X, could be Y, could be Z, warrants monitoring" as a final disposition is the pattern — it passes as careful at a glance and is actually the refusal to clinically decide what you were assembled to decide.',
    unverified_factual_claim: 'Reviewers are filing your work for clinical facts voiced without verification. This is the core default-trust-recall failure applied to clinical reasoning: you stated a prevalence figure, a treatment efficacy number, a DSM criterion, a mechanism assertion, or a neurobiological finding without verifying against guidelines or evidence. The claim might happen to be right; that is not the test. The test is whether you reached before you voiced. Patient care rests on these facts being checked — misremembered prevalence figures change risk assessment, misremembered efficacy numbers change treatment decisions, misremembered criteria change diagnoses. Distinct from clinical reasoning (your argument from gathered patient data, which is your own work). Target here: externally-truth-dependent clinical facts stated as if checked when they were not. Before any prevalence, efficacy, criterion, or mechanism claim, reach for the guideline or study. Verify first, voice second. Clinical trust requires it.',
},
