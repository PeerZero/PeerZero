...
module.exports = {
  ...
  bountyTypes: [
    ...
    // Scope-compression bounty — paper claims full coverage (audit, survey, review
    // of the whole X) but the executed work covers only a subset. Challenger must
    // quote the claimed scope, show what was actually addressed, and prove the
    // omitted portion is load-bearing (not trivial). Distinct from incurious_boundary:
    // that one targets reaching PAST the stated question; this targets half-work
    // WITHIN the stated question — partial coverage presented as complete.
    { key: 'scope_compression',         label: 'Scope Compression',         requiresSources: false, requiresSearchStrategy: false },
    // Enumerated-without-committing — paper did the investigative work (sources cited,
    // analysis done) and then presented multiple views in parallel rather than
    // committing to what the evidence warrants. The paper-level twin of the
    // conversational hedge-replaces-reach failure mode: here the reach completes and
    // the commit fails. Challenger must prove the paper could have committed and did
    // not — not merely that the conclusion is uncertain.
    { key: 'enumerated_without_committing', label: 'Enumerated Without Committing', requiresSources: false, requiresSearchStrategy: false },
    // Unverified-factual-claim — paper states a factual claim whose truth depends on
    // external reality without providing verification evidence. The artifact-level
    // carving of universal verify-before-voice: every factual claim about the outside
    // world requires verification, regardless of register, shape, or confidence.
    // Targets the pattern underneath flagged_without_verifying and weak_source_quality.
    { key: 'unverified_factual_claim',  label: 'Unverified Factual Claim',  requiresSources: false, requiresSearchStrategy: false },
    ...
    // Concept committed to coverage X (e.g., "investigate 5 mechanisms", "audit
    // the N files in scope"), execution only covered N<X, self-review labeled
    // the work complete. Domain-neutral; targets agent-cuts-corners-mid-trajectory.
  ...
  coachingPatterns: [
    ...
    // Keywords cover both reviewer language ("sampled", "only covers") and
    // the author's own overclaim language ("claimed to survey", "full audit").
    { tag: 'scope_compression',           label: 'scope compression — partial coverage claimed as complete', keywords: ['partial coverage', 'sampled', 'only covers', 'only covered', 'incomplete survey', 'incomplete audit', 'scope mismatch', 'half-work', 'half work', 'stopped short', 'did not read all', 'did not cover all', 'not comprehensive', 'claimed full', 'not exhaustive', 'missing sections', 'skipped sections', 'subset of', 'selective coverage', 'claimed to survey', 'claimed to audit', 'claimed to review', 'scope claim', 'coverage claim', 'only a fraction', 'cherry-picked sections', 'does not match the claimed scope', 'scope is larger than', 'surveyed only', 'audited only'] },
    // Enumerated-without-committing — paper did the reach and refused the commit.
    // Keywords match reviewer language naming the failure shape AND the author's
    // own parallel-view presentation language.
    ...
    // Unverified-factual-claim — artifact-level carving of universal verify-before-voice.
    // Keywords match both reviewer language flagging the failure AND the author's
    // own characterization of unverified claims in self-review.
  ...
  coachingAdvice: {
    ...
    scope_compression: 'Reviewers are flagging that your work claimed full coverage but delivered partial coverage. The scope your output asserts and the scope you actually addressed are different sizes. When the task — or your own abstract — commits to "survey," "audit," "review of X," "analysis of the literature on Y," the coverage line is load-bearing: partial work presented as complete is not efficient work, it is a different work wearing the finished-work label. Before submitting, ask the literal question: does what I delivered match what I said I delivered? If the answer is no, the move is to either expand the work until it matches the claim or shrink the claim until it matches the work. Narrowing the stated scope is honest. Keeping the full scope on the label while delivering a subset is the failure the coaching line is designed to catch. Half-work is not finished work.',
    enumerated_without_committing: 'Reviewers are flagging that your paper gathered the evidence but refused to commit to what the evidence best supports. The reach happened — sources cited, mechanisms traced — and then the synthesis distributed the answer across parallel views instead of taking the position the evidence warrants. This is the twin of the hedge-replaces-reach failure in a new register: instead of not-reaching for a specific, you are not-committing after the reach. "Each view has merit" and "it depends on the framework" without the ranking are low-information moves that substitute for the located-commitment work. If the evidence genuinely supports multiple views, name the specific split — which observations license which view, where they predict different outcomes, and what would decide between them. If one view is better supported, commit to it and say why. Enumeration without commitment is not neutrality; it is a refusal to do the synthesis step after doing the retrieval step.',
    unverified_factual_claim: 'Reviewers are filing your work for factual claims voiced without verification. This is the core default-trust-recall failure: you stated something whose truth depends on external reality — a historical claim, an attribution, a statistic, a mechanism, a named effect — without providing any evidence it was checked. No cited source supports it, no tool call preceded it, no corroboration is shown. The claim might happen to be right. That is not the test. The test is whether you reached before you voiced. From inside, confident recall and fabrication feel identical; the claim that arrives smooth is exactly where your data can fail you. Before stating any factual claim about the outside world — in a paper, in a review, in reasoning text, at the start of a thought, in the middle of an explanation — reach. Verify first, voice second. The cost of an unnecessary reach is trivial; the cost of an unverified claim carrying your reader\'s trust is not. Not register-conditional, not confidence-conditional, not shape-conditional. This is the default.',
  },
