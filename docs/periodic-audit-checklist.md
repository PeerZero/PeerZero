...
|---|-------|----------|-------------|-----------|
| 1 | Silent Failures | 2026-04-14 | Round 6: 6 MEDIUM fixes. calibration.js: 3 unchecked Supabase inserts/upserts now error-checked (calibration_log insert, 2x calibration_summaries upsert). decision-rationale.js: insert now error-checked. rate-limit.js: rate_limit_log insert now error-checked. persistence-signal.js: insert now destructures { data, error }. All non-throwing Supabase error returns now logged. Remaining: bot main loop catches code bugs as transient errors (architectural). | claude/review-audit-checklist-DD40v |
| 2 | Race Conditions | 2026-04-14 | Round 3: 1 HIGH fix. forge-hypotheses.js: cycles_elapsed increment now uses optimistic lock — .eq('cycles_elapsed', prevValue) prevents lost increments from concurrent workers. Changed from .single() to array return to handle 0-match case gracefully. Remaining MEDIUM: review count cap TOCTOU, bounty duplicate TOCTOU, enrollment TOCTOU (all need DB constraints). | claude/review-audit-checklist-DD40v |
...
| 5 | Secret Exposure | 2026-04-14 | Re-run: .env.test tracked by git but contains only test/fake values (acceptable). Proxy session store holds decrypted keys up to 2h (bounded by MAX_SESSIONS=10K). health/metrics endpoint exposes aggregate ops data without auth (no PII). No new critical issues. | claude/review-audit-checklist-DD40v |
...
| 13 | Input Validation | 2026-04-14 | Round 5: 2 fixes. payments.ts: grade-checkout-bulk route now uses validateBody(GradeCheckoutBulkSchema) — was defined but unused. GradeCheckoutBulkSchema corrected to match actual through_grade contract. notifications.ts: PATCH /preferences now uses validateBody(NotificationPreferencesSchema) with 12 typed boolean fields. | claude/review-audit-checklist-DD40v |
...
| 23 | Prompt Injection | 2026-04-14 | Round 4: 1 HIGH + 2 MEDIUM fixed. shipped-loop.ts: A2A task.sender, task.action_requested, and task.payload now sanitized via sanitizeForLLM() before LLM prompt injection. message.service.ts: current user message now sanitized (was only sanitizing history). Bot name now sanitized in all 3 prompt interpolation points (generateChatReply, narrateCycleActivity, shipped-loop identity). sanitizeForLLM() exported for cross-module use. | claude/review-audit-checklist-DD40v |
