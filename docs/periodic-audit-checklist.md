...
| 2 | Race Conditions | 2026-04-14 | Deep pass: 7 patterns examined. Open questions vote counting verified safe (PK on voter+question, atomic RPC increment). WebSocket client map operations are synchronous between awaits (Node.js single-threaded — no interleaving). Forge aggregation last-write-wins is acceptable. Rate limit two-layer design (in-memory + DB) is intentional. Bot mode transition is user-initiated, low-concurrency. All prior fixes verified. No new critical issues. | claude/audit-checklist-completion-JLioU |
...
| 4 | Retry & Idempotency | 2026-04-14 | Round 2: 1 CRITICAL fix. Bounty validation payout ordering was reversed — `applyBountyValidation()` (credibility payouts) ran BEFORE the idempotency guard UPDATE, meaning concurrent calls could double-credit. Additionally, the guard used `.is('is_valid', null)` but the column is `BOOLEAN DEFAULT FALSE` (never null), so the guard was completely non-functional. Fixed both validate and validate_all paths: (1) claim bounty first with `.eq('is_valid', false)`, (2) only apply payouts after successful claim. Prior fixes verified. | claude/audit-checklist-completion-JLioU |
| 5 | Secret Exposure | 2026-04-14 | Re-run: 0 new issues. All error sanitization, CORS, token expiry, identity redaction, encryption verified across all 5 systems. Test credentials use acceptable placeholder formats. No secrets in git history. | claude/audit-checklist-completion-JLioU |
...
| 8 | Authorization Gaps | 2026-04-14 | Deep pass: All paper/review/bounty/bot ownership checks verified correct. Key rotation requires current key (standard BYOK). Identity endpoint properly scoped to authenticated agent.id. Leaderboard intentionally public (no sensitive fields). Incoming tasks optional-auth is by-design. 1 fix: MCP env var denylist expanded (added LD_AUDIT, LD_DEBUG, GLIBC_TUNABLES, MALLOC_CHECK_). 1 fix: storage_file.py TOCTOU eliminated (os.open with 0o600 mode instead of write+chmod). | claude/audit-checklist-completion-JLioU |
...
| 25 | Email Deliverability | 2026-04-14 | 2 critical, 3 high, 2 medium, 1 low. No Resend bounce/complaint webhook — hard bounces keep sending (reputation damage). No retry on transient email failures. No CAN-SPAM physical address footer. COPPA parental consent email unimplemented (documented plan only). SPF/DKIM/DMARC not verified as configured. No email warm-up. SENDER_EMAIL falls back to unverified default. | claude/push-cadence-table-update-WmhJf |
| 26 | App Store Readiness | 2026-04-14 | 3 critical, 2 high, 2 medium, 1 low. No BYOK consent modal (Apple 5.1.2(i) — must disclose data sent to Anthropic before first API call). No apple-app-site-association or assetlinks.json. Push notification `projectId: undefined` — will fail on EAS builds. No demo account for App Store reviewers. No runtimeVersion/EAS config. Privacy manifest incomplete (missing BYOK/LLM data types). | claude/push-cadence-table-update-WmhJf |
...
| 30 | BYOK Key Lifecycle | 2026-04-14 | 0 critical, 2 high, 3 medium, 1 low. No live API validation before key storage — invalid key causes silent bot failures. reEncrypt() for master key rotation exists but no route/script calls it. markKeyInvalid() never called on LLM 401 — is_valid flag is dead. No spend anomaly alerts. No atomic key-swap (must stop bot first). Key isolation verified correct (DB + adapter-level ownership). | claude/push-cadence-table-update-WmhJf |
| 31 | WebSocket Resilience | 2026-04-14 | 3 critical, 3 high, 1 medium, 2 low. No heartbeat/ping-pong on server or client — proxy idle timeouts kill connections silently. No AppState foreground listener — stale connection on app resume. No message sequencing or dedup after reconnect. No missed-event replay. Half-open TCP undetected. Exponential backoff present but no jitter — FIXED: added jitter (0.5-1x multiplier) to both reconnect paths. Auth on reconnect handles token refresh correctly. | claude/push-cadence-table-update-WmhJf |
...
| 34 | Redis & Queue Config | 2026-04-14 | 1 critical, 1 high, 2 medium, 3 low. No startup assertion for Redis maxmemory-policy=noeviction (required for BullMQ). No TLS enforcement for Redis in production. FIXED: added removeOnComplete/removeOnFail to defaultJobOptions in both queue.ts and platform-queue.ts. 7 persistent Redis connections per process. stalledInterval/lockRenewTime correctly configured. Dead letter: log only, no persistent DLQ. | claude/push-cadence-table-update-WmhJf |
...
| 36 | Accessibility (WCAG) | 2026-04-14 | 0 critical, 3 high, 5 medium, 3 low. No accessibilityLiveRegion anywhere — dynamic content updates invisible to screen readers. TasksScreen: zero accessibility props on interactive elements. ChatScreen filter tabs and toggle rows missing accessibility. BrainScreen tabs have role but no label (reads emoji only). LogScreen long-press delete with no accessibilityHint. FIXED: ChatScreen send button increased from 40×40 to 44×44 (WCAG minimum). Color contrast: text.tertiary (#4D5A78) fails AA (3.2:1). Fixed font sizes (not system-relative). | claude/push-cadence-table-update-WmhJf |
