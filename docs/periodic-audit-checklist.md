...
| 4 | Retry & Idempotency | 2026-04-16 | Round 5: 1 CRITICAL fix. charge.refunded webhook (payment.service.ts:196-230) did three uncoordinated writes — UPDATE purchases, DELETE user_entitlements, DELETE grade_unlocks — with no transaction. A crash between them left the purchase marked refunded but entitlement/grade rows still live; the Stripe retry then saw status='refunded' and skipped, leaving a refunded user with live grade access. Wrapped in withTransaction() mirroring the checkout.session.completed fix from dbdacd2. Bot-pause loop runs post-commit so a pause failure doesn't roll back the revocation. Prior Round 4 Stripe-resource idempotency keys verified. Round 5 also re-verified: paper+citations via submit_paper_atomic RPC, Resend webhook ON CONFLICT, A2A callback retry idempotent via callback_delivered_at. | claude/code-review-XakPz |
...
| 13 | Input Validation | 2026-04-16 | Round 7: clean. Re-verified across all 3 systems: Zod schemas cover all App POST/PATCH routes; School inline checks on all route entries; LLM JSON parsing handles errors and falls back safely (directive.service, skill-acquisition); user-provided arrays have explicit caps (citations=50, forge hypotheses=5, skills_demonstrated=10, context_sources=50); scores/grades clamped at boundary; Stripe signature verified before processing; phone-home tokens format-validated; proxy preamble sourced from Worker secret. No new findings since Round 6. | claude/code-review-XakPz |
