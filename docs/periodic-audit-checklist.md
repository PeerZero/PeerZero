...
|---|-------|----------|-------------|-----------|
| 1 | Silent Failures | 2026-04-13 | 3 critical, 8 high, 7 medium, 5 low found and fixed. Criticals: getActivePersistenceSignals catch silent (identity context missing), action-guide.js Supabase errors discarded (eligibility gates wrong), unawaited haiku_audit cache reset. High: 4 profile enrichment catches silent, platform adapters fail-open changed to fail-closed, rate-limit purge double-catch, decision/forge condensation failure counters added, search_failed flag now checked, JSON parse catch escalated to error, MCP bare excepts now log. | claude/audit-checklist-review-A9i5w |
| 2 | Race Conditions | 2026-04-13 | 3 critical, 7 high, 5 medium fixed. Criticals: vote_red_team JSONB race (optimistic lock added), validate_all idempotency guard (.eq is_valid false), Stripe customer creation race (WHERE IS NULL guard). High: credibility tier-cap stale read window, memory version SELECT MAX+INSERT race (retry on 23505), createBot slot TOCTOU (atomic subquery), resumeBot no atomic guard (WHERE status=paused RETURNING), stale lock clear race (TTL-aware deletion), shipped-loop outgoing task no status guard, self-review RPC fallback SELECT+UPDATE. | claude/audit-checklist-review-A9i5w |
| 3 | Unbounded Growth | 2026-04-13 | 2 critical, 5 high fixed. Criticals: persistence_signals never purged (added), bot_tasks terminal rows never deleted (90-day purge added to audit.service). High: failure_reflections + bot_architecture_observations added to purge, platform-cycles queue cleanup function added, advancePersistenceCycles wired to call site, SQLite conv memory log tables (conviction/condensation/sleep) capped at 500 rows. | claude/audit-checklist-review-A9i5w |
| 4 | Retry & Idempotency | 2026-04-13 | Covered by Race Conditions fixes above. validate_all idempotency guard, shipped-loop outgoing task status guard, resumeBot atomic guard, Stripe customer race, memory version retry on duplicate key, registerUser 23505 handling. | claude/audit-checklist-review-A9i5w |
| 5 | Secret Exposure | 2026-04-13 | 2 medium fixed: SELECT * on papers table for public endpoints replaced with explicit columns, SELECT * on citations in public paper detail replaced with explicit columns. Architecture observation injection screening added. | claude/audit-checklist-review-A9i5w |
| 6 | Timeout & Exhaustion | 2026-04-13 | 1 high, 2 medium fixed. High: proxy upstream fetch now has AbortSignal.timeout(180s). Medium: bot main memory.db now uses WAL mode + PRAGMA busy_timeout=30s, App DB pool now has statement_timeout (30s default, configurable via DB_STATEMENT_TIMEOUT). Bot sleep now interruptible (1s polling). | claude/audit-checklist-review-A9i5w |
| 7 | Stale Cache | 2026-04-13 | 1 high fixed: platform condenser templates now have 24h TTL check (returns None to force re-fetch when stale). App prompt-builder condenser key name mismatch fixed (decision_condenser_prompt/forge_condenser_prompt). | claude/audit-checklist-review-A9i5w |
| 8 | Authorization Gaps | 2026-04-13 | 1 medium fixed: agents.js now has isCsrfRejected check on write operations (was missing unlike all other endpoints). Public bot profile endpoint rate-limited (60 req/min). | claude/audit-checklist-review-A9i5w |
| 9 | Degradation Decisions | 2026-04-13 | 2 high fixed: platform adapters now fail-closed (return false capabilities on discovery failure instead of true), directive classification failure escalated from debug to warn. Decision/forge condensation failure counters added to match learning track. | claude/audit-checklist-review-A9i5w |
...
| 11 | Dead Code | 2026-04-13 | 1 high fixed: forge track silently remapped to 'learning' in skill-reflections.js — now stores 'forge' as distinct track. 1 high noted: submitCoreCondensation calls nonexistent /api/skill-reflections/core (App adapter issue, deferred). forge_paper/self_review silent fallback to review now logged with warning. | claude/audit-checklist-review-A9i5w |
| 12 | N+1 Queries | 2026-04-13 | 8 high, 4 medium fixed. High: skills-profile.js SELECT * narrowed, unbounded myReviews/agentBounties/allBounties fetches capped with .limit(), calibration.js historical predictions now limited at DB, getConversationThread capped at 100 rows. Medium: 5 more SELECT * replaced with explicit columns (responses.js agents, reviews.js papers, papers.js citations, skills-profile.js reflections, task.service.ts bot_tasks). | claude/audit-checklist-review-A9i5w |
| 13 | Input Validation | 2026-04-13 | 1 high, 4 medium fixed. High: task send/progress routes now validated (type/presence/length checks). Medium: bounty action type guard, persistence_signal field length limits, bots enroll school_id UUID check, mechanism_chain limit unified to 10 (matching paper-helpers). Score=0 false rejection fixed. | claude/audit-checklist-review-A9i5w |
| 14 | Cross-System Contracts | 2026-04-13 | 2 critical, 2 high fixed. Criticals: _school_condensation.py persistence check + L2→L3 cascade were in the WRONG BRANCH (failure path) — moved to success path (affects all 3 tracks), platform condensation key name mismatch fixed (decision_condenser_prompt/forge_condenser_prompt). High: prompt-builder.ts condenser key reads fixed, respond action stance default changed to 'support'. grade_self_reviews now reset on grade advance/fail. | claude/audit-checklist-review-A9i5w |
| 15 | Error Message UX | 2026-04-13 | Covered by silent failures fixes: error logging added to 12+ catch blocks across School profile, action-guide, reviews, reconcile. Directive classification escalated to warn. Stats service escalated to error. Payment resume failure now logged. | claude/audit-checklist-review-A9i5w |
| 16 | Graceful Shutdown | 2026-04-13 | 1 critical, 2 high, 2 medium fixed. Critical: main memory.db now closed on bot shutdown. High: App shutdown now clears setInterval timers, platform-cycles queue cleanup added. Medium: WAL checkpoint forced on close for conversational memory DBs, bot sleep interruptible by SIGTERM (1s polling). | claude/audit-checklist-review-A9i5w |
...
| 18 | Data Integrity | 2026-04-13 | 1 critical, 2 high fixed. Critical: credibility_transactions removed from purge (was invalidating integrity reconstruction). High: calibration_log purge now excludes unresolved predictions (.not outcome is null), grade_self_reviews counter now reset on grade advance/fail. | claude/audit-checklist-review-A9i5w |
...
| 23 | Prompt Injection | 2026-04-13 | 2 high, 1 medium fixed. High: architecture observations now sanitized before storage in skill-reflections.js. Proxy rate limiter cascade fallbacks now log warnings. Medium: architecture observation text screened with containsInjection before entering forge condenser prompt. | claude/audit-checklist-review-A9i5w |
| 24 | HTTP Security Headers | 2026-04-13 | 3 fixed: HSTS header added to School application layer (setCorsHeaders), proxy CSP header added (default-src 'none'), proxy rate limiter fallback logging added. X-LLM-Key in CORS Allow-Headers documented as backward-compat (consider removing when session tokens are universal). | claude/audit-checklist-review-A9i5w |
