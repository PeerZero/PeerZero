...
| 13 | Input Validation | 2026-04-15 | Round 6: parseInt radix parameter added to all server-side parseInt calls across School (calibration.js, tier-display.js, doi-citations.js) and App (db/client.ts, routes/bots.ts). Bot agent.py: KeyError added to review score clamping except clause. Bot llm_client.py: OpenAI choices[0] access hardened against empty arrays (safe .get() chain). | claude/audit-code-launch-prep-5xshZ |
