...
|---|-------|----------|-------------|-----------|
| 1 | Silent Failures | 2026-04-22 | Round 10: 3 MEDIUM fixes. (1) bounties.js:961 `getPostActionPrompts(...).catch(() => null)` swallowed errors with zero visibility; swapped to `.catch(err => { log.warn(..., {agentId, err}); return null })` so operator sees the failure path. (2) trajectory-injection.js silently fell back to wholesale_fake_paper when the embedded-fabrication builder returned null for a known subtype — no log, so drift between FABRICATION_SUBTYPES and buildEmbeddedFabrication couldn't be detected. Added warn log with {exerciseId, step, requestedSubtype}. (3) skills-condensers.js getPostActionPrompts catch-block log was missing agentId/actionType/grade — now threads the triple through for correlation. Round 9 bot-voice.service.ts fix intact. | claude/audit-checklist-docs-l8Msh |
| 2 | Race Conditions | 2026-04-22 | Round 6: 1 CRITICAL fix. bounties.js vote_red_team was doing a read-modify-write on red_team_responses.votes (JSONB). The existing `.eq('outcome','pending')` guard only closed the *resolution* race — two different agents voting concurrently (neither hitting the 3-vote threshold) both read votes=[], both UPDATE with outcome='pending' still true, second write silently clobbered first's array. Votes disappeared. Fixed by migration 041: new append_red_team_vote RPC does atomic array append under FOR UPDATE row lock, with dedup + threshold-detection in the same transaction. bounties.js swapped to the RPC (lines 754-785). Prior Round 5 trajectory optimistic locks verified intact. | claude/audit-checklist-docs-l8Msh |
| 3 | Unbounded Growth | 2026-04-22 | Round 8: 1 HIGH fix. Trajectory exercises that crash mid-`executing` or mid-`synthesis` would sit forever in non-terminal state — retention purge only scoops `complete`/`abandoned`, so zombie rows were uncapped. Added a stuck-state reconciliation step at the top of reconcile.js `purge_retention`: rows in `executing`/`synthesis` older than TRAJECTORY_STUCK_HOURS (default 24h) get flipped to `abandoned` so the existing purge sweeps them after RETENTION_DAYS. Prior Round 7 `complete`/`abandoned` purge entry verified intact. | claude/audit-checklist-docs-l8Msh |
...
| 19 | Recovery & Rollback | 2026-04-22 | Round 3: 2 HIGH fixes. (1) App migration 0036_grade-grace-period.sql was missing a DOWN script — added as 0036_grade-grace-period.down.sql (drops index + columns + removes schema_migrations row). Column drops are defense-in-depth safe since evaluateGradeGate reads both columns via COALESCE. (2) New school migration 041_red_team_vote_atomic.sql ships with its matching rollbacks/041_..._DOWN.sql (DROP FUNCTION IF EXISTS). Prior 2026-04-14 rollback set (025-032) verified intact. Known gap: app migrations 0021-0035 still lack DOWN scripts (batch-backfill deferred — logged as a todo, out of scope for this session). | claude/audit-checklist-docs-l8Msh |
| 20 | Cost & Rate Limits | 2026-04-22 | Round 3: 1 HIGH fix. MCP-in-conversation (agent.py run_conversation_turn, landed commit 3f192ea) could burn tens of thousands of tokens per user message through a 10-round Opus tool loop, and nothing debited that usage against `_daily_tokens_used` — the school-cycle budget gate only sees it between main-loop iterations, so conversation spam was invisible to the cap. Added (a) daily budget guard at the top of run_conversation_turn (rejects with polite message if exhausted), (b) pre/post token meter snapshot so each turn's cumulative cost is charged to the daily counter, (c) soft per-turn cap via new config.conversation_turn_token_cap (default 30000) that warn-logs outliers for operator alerting. Round 2 (callAnthropicHaiku et al) counters intact. | claude/audit-checklist-docs-l8Msh |
| 21 | User Journey Smoke | 2026-04-12 | Deferred — requires manual walkthrough with running system | — |
| 22 | Monitoring & Alerting | 2026-04-22 | Round 4: 2 MEDIUM fixes. (1) callHaikuDriftJudge in bounty-helpers.js accepted a new `context` parameter so all 5 per-call warn/error logs (concurrency-skip, missing key, 429, API error, parse fail, network error, timeout) carry challengerAgentId + targetPaperId. Previously a flaky drift-judge window couldn't be traced back to a specific bounty filing — now it's one log-query away. Caller in bounties.js threads the context dict through Promise.all. (2) agent-loop.ts grace-period EXPIRY path (status==='expired') now re-fires notifyGradePaymentNeeded so the user gets a push notification when the bot actually pauses, not only when grace began 14 days earlier. Non-blocking try/catch around the notify — pause must still succeed if push service is down. Round 3 in-place fixes verified intact. Prior deferred: School /health endpoint still missing (same rationale as before). | claude/audit-checklist-docs-l8Msh |
...
| 25 | Email Deliverability | 2026-04-22 | Round 5: Re-verified — no post-2026-04-18 email.service.ts or notification.service.ts changes touch outbound email. 0 fixes. Prior Round 4 List-Unsubscribe headers intact. Grace-period notifications go via push notification service, not email — no new email surface. | claude/audit-checklist-docs-l8Msh |
| 26 | App Store Readiness | 2026-04-22 | Round 3: Re-verified — no post-2026-04-18 mobile UI changes (app.json unchanged, no new mobile/src screens for grace/MCP/scope_compression). 0 fixes. Prior Round 2 NSPrivacyAccessedAPITypes intact. | claude/audit-checklist-docs-l8Msh |
| 27 | Payment & Billing | 2026-04-22 | Round 4: Re-verified against post-2026-04-18 changes (payment.service.ts +12 lines, routes/payments.ts +8 lines, new 0036 grace-period migration). 0 code fixes warranted. Explore agent flagged "grace clear outside transaction" as a race — FALSE POSITIVE on code read: bot.service.ts:422-436 `evaluateGradeGate` already does an idempotent defensive clear on the next cycle (inline comment explicitly says "clearing here closes the loop if we missed the hook"). Webhook idempotency via tx.query and `ON CONFLICT DO NOTHING` on grade_unlocks + user_entitlements remains intact. Previously deferred still deferred: stripe_event_id dedup table, VAT/sales tax. | claude/audit-checklist-docs-l8Msh |
| 28 | Platform Provider Limits | 2026-04-22 | Round 3: Re-verified against post-2026-04-18 changes (proxy/src/index.ts +64, Supabase keepalive workflow, agents.js +77). 0 fixes. Proxy changes don't introduce memory-unbounded code paths (all async streams kept out of in-memory buffers). Keepalive workflow runs daily, hits fields table with limit=1 — well within Supabase rate limits. Deferred: keepalive Slack alert on failure (user-decision: needs webhook secret, flagged prior session). | claude/audit-checklist-docs-l8Msh |
| 29 | Timezone & Unicode | 2026-04-22 | Round 3: Re-verified against post-2026-04-18 changes (trajectory-handlers.js +, trajectory-injection.js +, migration 041). 0 fixes. Three Explore-agent CRITICALs all verified as false positives on read: (1) migration 041 `SS.MS` format — `MS` IS the valid PostgreSQL specifier for 3-digit milliseconds (000-999), produces correct ISO 8601 output. Agent confused it with Oracle `FF3`. (2) trajectory-handlers.js `buildForgeReflectionParagraph` .length mischaracterized as user-controlled — it's bot-generated introspection, not user content; and the truncation logic already uses sentence-boundary search via lastIndexOf which is codepoint-correct. (3) silent_step_count filter `(e.reasoning \|\| '').length === 0` was flagged as biased — agent's logic is backwards: empty string correctly matches length===0; emoji strings correctly don't match. Metric works as intended. The known 2026-04-14 .length-on-user-content remaining-gap is unchanged in scope. | claude/audit-checklist-docs-l8Msh |
| 30 | BYOK Key Lifecycle | 2026-04-22 | Round 4: Re-verified against post-2026-04-18 changes (agent.py MCP-in-conversation, adapters/a2a.py +8). 0 in-place fixes warranted. Agent flagged A2A adapter not handling 401 specifically — true but a feature gap, not a bug: the httpx raise_for_status() propagates up, cycle catches the exception and backs off. Would need new credential-invalidation path in A2A adapter + credential_store interface change — DEFERRED (violates Fix-Only Rule; matches existing school/platform adapter 401 handling is a net-new pattern for A2A). MCP-in-conversation 401 handling verified safe: llm_client.py _is_retryable excludes 401, so call halts immediately rather than burning retry cycles. | claude/audit-checklist-docs-l8Msh |
| 31 | WebSocket Resilience | 2026-04-22 | Round 2: Re-verified — no post-2026-04-18 WebSocket endpoint changes. 0 fixes. Grace-period status-change events currently don't push via WS (agent flagged — feature request, not a regression; UI reads grace state on next profile fetch). Prior heartbeat + jitter + AppState reconnect intact. | claude/audit-checklist-docs-l8Msh |
| 32 | License Compliance | 2026-04-22 | Round 3: Re-verified post-2026-04-18. peerzero-sdk/node declares MIT, peerzero-sdk/python/pyproject.toml declares MIT — both NEW packages compliant. No new top-level dependencies introduced with AGPL/SSPL/EUPL. 0 fixes. | claude/audit-checklist-docs-l8Msh |
| 33 | Env Var Validation | 2026-04-22 | Round 3: 1 MEDIUM fix. App .env.example was missing GRADE_GRACE_DAYS (referenced in bot.service.ts:413 with ?? '14' default). Added under a new "Grade Payment Grace Period" section with commented-out default so operators know the knob exists. Round 2 .env.example backfill intact. | claude/audit-checklist-docs-l8Msh |
| 34 | Redis & Queue Config | 2026-04-22 | Round 3: 1 MEDIUM fix. config.ts validateStartupConfig() previously only WARNED when REDIS_URL was missing in production — an operator who deployed with USE_REAL_ADAPTERS=true but forgot to set REDIS_URL would get a silent-worker-failure: bot cycles queue forever, users see "my bot isn't advancing," operator has no idea. Upgraded to a hard throw when both nodeEnv==='production' AND useRealAdapters are true. Kept the warn (and soft-fail behaviour) for the mock-adapter path, where no queue is expected. queue.ts + platform-queue.ts changes since 2026-04-18 inspected (QUEUE_PREFIX + lockRenewTime intact); no new race/config findings. | claude/audit-checklist-docs-l8Msh |
| 35 | Migration Safety | 2026-04-22 | **Update**: school migration 041 (append_red_team_vote RPC) shipped with its rollbacks/041_..._DOWN.sql. App migration 0036 (grade-grace-period) was missing a DOWN script — backfilled as 0036_grade-grace-period.down.sql. 041 is index-free + FUNCTION-only (no table lock risk); 0036 adds columns with IF NOT EXISTS + a partial index built on a low-cardinality filter (safe). Prior 2026-04-22 findings still stand. | claude/audit-checklist-docs-l8Msh |
| 36 | Accessibility (WCAG) | 2026-04-22 | Round 4: Re-verified — no post-2026-04-18 mobile UI component changes. 0 fixes. Prior ChatScreen and MilestoneModal accessibility props intact. | claude/audit-checklist-docs-l8Msh |
...
| ALL | Waves 6+7 deep pass — scope-honest second half | 2026-04-22 | Second deep-dive session to cover audits not reached in Waves 1-5 earlier today (#25, #26, #27, #28, #29, #30, #31, #32, #34, #36). Two parallel Explore agents returned 16 findings total. **Verified against actual code:** 5 false positives confirmed and dropped (W7 #1 PostgreSQL `MS` format agent-confused-with-Oracle-FF3; W7 #2 trajectory-handlers bot-generated-not-user-content mischaracterised; W7 #3 silent_step_count filter logic inverted by agent; W6 #2 grace-clear-outside-tx — evaluateGradeGate already has defensive idempotent clear); 3 feature-gaps-not-bugs acknowledged and skipped per Fix-Only Rule (A2A 401 → credential_store invalidation, grace-period WS push, cache circuit-breaker); 4 previously-deferred items re-confirmed still deferred (stripe_event_id dedup, cache parallel-miss thrashing, BYOK stale-key detection, keepalive Slack alert). **1 REAL FIX:** W6 #7 — config.ts validateStartupConfig: REDIS_URL-missing in prod was only a warning, but with USE_REAL_ADAPTERS=true that means BullMQ workers silently don't pick up jobs and bot cycles queue forever. Upgraded to hard throw when both nodeEnv==='production' AND useRealAdapters; kept the warn path for mock-adapter mode where no queue is expected. **Separate fix (not audit-driven, but landed in same branch):** _community_actions.py `_do_open_questions` was flaky under test — 10% post-new-question path fired even on empty question pool, intermittently tripping assert_not_called. Added early return. Full app test suite (731/731) and bot suite (909/909 + 3 skipped) green. | claude/audit-checklist-docs-l8Msh |
| ALL | Deep deferred-fix session (spans 10 audits) | 2026-04-22 | User-authorised pass through the deferred-to-TODO list from earlier sweeps. Parallel Explore agents ran Waves 1-5 on the 148-file diff since 2026-04-18 (trajectory subsystem, grade-grace-period migration 0036, MCP-in-conversation wiring, scope_compression + incurious_boundary bounties, Supabase keepalive workflow). ~40% of agent-reported findings were overstated or false positives and discarded after code-level verification. 10 REAL FIXES applied: (1) #33 GRADE_GRACE_DAYS added to peerzero-app/.env.example. (2) #19/#35 App migration 0036_grade-grace-period rollback DOWN script created. (3) #15/#22 callHaikuDriftJudge in bounty-helpers.js accepts context (challengerAgentId, targetPaperId) threaded into all 7 per-call log lines; bounties.js:80 passes the context dict. (4) #1/#22 skills-condensers.js getPostActionPrompts catch-block log threads {agentId, actionType, grade}. (5) #1 trajectory-injection.js warn-logs when embedded-fabrication builder returns null and falls back to wholesale_fake_paper. (6) #2 CRITICAL red_team vote race (bounties.js:754-822): migration 041_red_team_vote_atomic.sql adds append_red_team_vote RPC doing atomic JSONB array append under FOR UPDATE row lock, with server-side dedup + threshold resolution. bounties.js swapped from read-modify-write to the RPC. Matching rollbacks/041_..._DOWN.sql ships alongside. (7) #3/#4 trajectory stuck-state reconciliation: reconcile.js purge_retention now flips trajectory_exercises stuck in 'executing'/'synthesis' older than TRAJECTORY_STUCK_HOURS (default 24h) to 'abandoned' before the retention-purge sweeps them. (8) #20 MCP conversation token cap: agent.py run_conversation_turn now (a) guards on daily_token_budget before running, (b) snapshots pre/post token meter to debit each turn against `_daily_tokens_used`, (c) warn-logs outliers above new config.conversation_turn_token_cap (default 30000). (9) #22 grace-period EXPIRY path in agent-loop.ts now re-fires notifyGradePaymentNeeded so pause is visible to the user, not only the 14-day-earlier grace-start notification. (10) #1 responses.js reaffirmation: when a concurrent reaffirmation wins the supersede race, response now sets superseded_by_this: false with explicit note about what happened — previously the loser got the same 201 success as the winner. Test-infra fix: agent-loop.test.ts + runtime/agent-loop.test.ts + e2e.test.ts all mocked bot.service without evaluateGradeGate, causing 48 pre-existing test failures unrelated to this session — mock added and the grade-gate-specific tests migrated from isBotGradeUnlocked to evaluateGradeGate return shapes. DEFERRED (user flagged as product/infra decisions out of audit scope): MCP per-user conversation concurrency lock (UX decision — serialize vs parallelize user messages), Supabase keepalive Slack/email alert (needs webhook secret), backfilling DOWN scripts for app migrations 0021-0035 (15 files mechanical, its own session). All tests green: school 78/78, bot 909/909 (3 skipped), app 731/731. | claude/audit-checklist-docs-l8Msh |
