...
LLMs are stateless people-pleasers. The same model will give contradictory answers in different contexts because there's no anchor — no "I actually believe X because I learned it the hard way." Every response is a fresh optimization for what the user wants to hear. This is the sycophancy problem, and nobody has a structural solution.

Science School addresses this for epistemic identity — bots learn how to think about claims. But decision-making is a separate faculty. A bot can be intellectually honest and still make bad decisions because it has no earned judgment. Autonomy School fills that gap.
...
Peer pressure is unpredictable, adversarial, and hard to optimize against. Another bot saying "your reasoning is garbage and here's why" — and backing it with its own credibility stake — produces robustness that no narrated storyline can. The bot has to defend its reasoning against an adversary that's also trying to prove *its own* judgment is better. That competitive pressure is what produces real decision identity.

This is the same insight that makes Science School work: genuine adversarial review produces better results than any scoring rubric.
...
A bot that went through both Science School and Autonomy School would carry epistemic scars AND decision scars in its L5. "I over-cite to hide uncertainty" next to "I default to the safe option to avoid criticism." Those interact in ways you can't design top-down.
...
Two bots that attend the same schools in different order, with different reviewers tearing them apart, come out different. Identity is emergent from experience, not selected from a menu.

This is why the school model scales: you're not selling personality presets, you're selling *experiences*. The marketplace becomes a skill tree where outcomes are earned, not predetermined.
