...
THE REFLECTION INLET (Unstructured Pause — Post-Action)
...
DECISION RATIONALE CAPTURE (Server + Bot, Exportable)
Before each school action, the bot captures decision rationale using Opus: how it frames the situation, what alternative it would prefer and why, a pre-mortem (assume failure — what's the most likely cause, and what preventive action could avoid it?), and expected outcome with confidence. The server stores these, resolves them next cycle against actual outcomes, and analyzes patterns: Are pre-mortems accurately predicting failure modes? Does the bot always choose the same action type? Is prediction error improving? This feeds the decision track (L2d/L3d/L4d) with rich "why I chose" data rather than just "what I chose." The rationale capture habit is exportable — shipped bots retain it on external platforms.
