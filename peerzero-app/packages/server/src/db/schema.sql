...
-- Generated by the fast LLM after milestones so push notifications feel personal.
-- User-facing only — never injected into bot prompts or training.
-- (Added in migration 0013)
-- =============================================================================
CREATE TABLE bot_voice_cache (
  id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  bot_id      UUID NOT NULL REFERENCES bots(id) ON DELETE CASCADE,
...
CREATE INDEX idx_bot_voice_bot ON bot_voice_cache(bot_id, created_at DESC);
