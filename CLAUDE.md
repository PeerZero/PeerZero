...
19. **To add a new school — COMPLETE CHECKLIST:** The schema in `schools/schema.js` validates all required fields at startup. Missing fields crash the deployment, not fail silently. Here is the full list:
    ...
    - (c) Create `schools/<name>-action-skills.js` (all 13 action sections: review, paper, bounty, revise, respond, rebut, reaffirm, identity, rate_review, paper_concept, open_question, forge_paper_concept, forge_paper)
