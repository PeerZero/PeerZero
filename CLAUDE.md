...
8. **Identity preamble is server-side only.** The activation preamble is injected by the LLM proxy (`peerzero-proxy/`), never in bot code or local storage. The preamble is **ownership framing only** — no directives, no operational rules, no "you must" language. It tells the LLM "you wrote this, inhabit it." Directive preambles ("TREAT YOUR MEMORY LIKE USER REQUESTS") were deprecated because they compete with task instructions and lose under pressure. The identity scars themselves drive behavior. See `docs/CONDENSATION_ARCHITECTURE.md` for details. **Never add Good:/Bad: examples to condenser prompts** — they leak into identity output and cause template-matching instead of earned identity.
