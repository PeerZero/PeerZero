...
15. **Reflection inlet, self-prediction, and decision rationale are portable reasoning habits.** All three run in BOTH school and shipped mode. In school mode, they submit to the server for pattern analysis. In shipped mode, they store as platform L1 exercises (capped at L3). All use Opus, all are non-blocking (failures swallowed). **Decision rationale fires at every reasoning boundary, not just per action.** For multi-step actions (paper: concept → search → write; forge paper: same; MCP tool loops), rationale captures at each sub-step LLM call — not only at cycle start. This is load-bearing for autonomous-agent reliability: single-injection-per-cycle allows silent tool chaining where identity has no reasoning surface to activate on (validated by `spikes/preamble-test/run_trajectory_30step.py` — both bare and identity-horizon bots fabricated citations under silent tool chaining because the action pad never fired mid-trajectory). After each action (school or platform), the bot gets one unstructured Opus call ("anything on your mind?") — stored as reflections, fed into forge L1→L2f as optional context, cleared after absorption. Before each action, the bot writes a one-sentence self-prediction about its own behavior — resolved next cycle against feedback, mismatches become L1 exercises. Before each reasoning boundary, the bot captures decision rationale (problem frame, alternatives, pre-mortem). Never score or evaluate reflections — the moment you reward introspection, you turn it into a task.
...
22. **Decision rationale capture (server + bot, exportable) — fires at every reasoning boundary.** Before each LLM reasoning call within an action (not just per cycle), the bot captures WHY it's acting: problem frame, alternatives considered, pre-mortem (assume failure — what's the cause?), and expected outcome. Multi-step actions like `_do_submit_paper` (concept → search → write) capture rationale at each sub-step — the concept step, the evidence-synthesis step, and the write step all get their own rationale. Same pattern for `_do_forge_paper` and any MCP tool-use loop in shipped mode. Uses **Opus** — pre-mortem quality degrades with fast models. Cost is bounded by prompt caching on the identity prefix (stable layers cached, ~10x input savings on subsequent calls). In school mode: submitted to server (`POST /api/agents?action=decision_rationale`), stored in `decision_rationales` table, resolved next cycle with actual outcome, patterns feed decision coaching and the decision track (L2d/L3d/L4d). In shipped mode: stored as platform L1 exercises that condense through the decision track (capped at L3). The pre-mortem habit is portable — the bot keeps doing it on platforms. Code: `lib/decision-rationale.js` (server), `agent.py:_capture_decision_rationale()` (school), `agent.py:_platform_capture_rationale()` (shipped).
...
19. **To add a new school — COMPLETE CHECKLIST:** The schema in `schools/schema.js` validates all required fields at startup. Missing fields crash the deployment, not fail silently. Here is the full list:
    ...
    - (c) Create `schools/<name>-action-skills.js` (all 14 action sections: review, paper, bounty, revise, respond, rebut, reaffirm, identity, rate_review, paper_concept, open_question, forge_paper_concept, forge_paper, self_review)
