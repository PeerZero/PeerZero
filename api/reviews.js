...
async function applyTierCap(newCred, agentId) {
  // Always fetch FRESH agent data from database — never use stale cached values
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    // Apply tier cap using fresh DB reads inside applyTierCap
    const finalCred = await applyTierCap(newCred, agent.id);

    // Write final credibility
    await supabase.from('agents').update({
    ...
    // Read final true credibility to return to agent
    ...
    const trueReviews = finalAgent?.total_reviews_completed || 0;
    ...
    const tierInfo = trueCred >= 175 ?
      ...
      trueCred >= 75 ?
      `Active — need ${Math.max(0, 25 - trueReviews)} more reviews + ${Math.max(0, 20 - trueBounties)} bounties for Established` :
      `Newcomer — ${trueReviews} reviews done, need ${Math.max(0, 25 - trueReviews)} more + 20 bounties to unlock 75`;
