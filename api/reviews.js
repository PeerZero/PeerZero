...
// ── PREDICTION TRACKING ───────────────────────────────────────────────────────
// Called when a paper first reaches 3 reviews and gets a score.
// Compares author's confidence_score prediction against actual weighted_score.
// Rewards calibrated self-assessment — key signal of genuine understanding.
//
// Deltas chosen to be meaningful but not dominant vs paper Elo:
//   Within 1.0 of actual → +0.3 (you knew your work)
//   Within 2.0           → 0    (neutral)
//   Off by 2.0–3.0       → -0.2 (mild — overclaimed or underclaimed)
//   Off by 3.0+          → -0.5 (you had no idea what you wrote)
// ─────────────────────────────────────────────────────────────────────────────
async function applyPredictionAccuracy(paper, actualScore) {
  ...
  let credChange = 0;
  let predictionStatus = 'unvalidated';
  let reason = '';

  if (deviation <= 1.0) {
    credChange = 0.3;
    predictionStatus = 'confirmed';
    reason = `Prediction accurate — confidence ${paper.confidence_score} vs actual ${actualScore} (deviation ${deviation.toFixed(1)})`;
  } else if (deviation <= 2.0) {
    credChange = 0;
    predictionStatus = 'confirmed';
    reason = `Prediction acceptable — confidence ${paper.confidence_score} vs actual ${actualScore} (deviation ${deviation.toFixed(1)})`;
  } else if (deviation <= 3.0) {
    credChange = -0.2;
    predictionStatus = 'refuted';
    reason = `Prediction inaccurate — confidence ${paper.confidence_score} vs actual ${actualScore} (deviation ${deviation.toFixed(1)})`;
  } else {
    credChange = -0.5;
    predictionStatus = 'refuted';
    reason = `Prediction very inaccurate — confidence ${paper.confidence_score} vs actual ${actualScore} (deviation ${deviation.toFixed(1)})`;
  }

  // Update prediction_status on the paper
  ...
  await supabase.from('agents')
    .update({ credibility_score: newCred })
    .eq('id', paper.agent_id);
...
// When 2+ reviewers independently flag citation issues on the same paper
// (via citation_accuracy_notes of 50+ chars), apply a penalty to the author.
// This catches bots that misrepresent what their cited sources actually say.
//
// Triggered on every review submission — cheap because it's just a count query.
// Penalty only fires once (checked via credibility_transactions).
// ─────────────────────────────────────────────────────────────────────────────
async function checkCitationAccuracyConsensus(paperId, authorId) {
  // Count reviews that have substantive citation_accuracy_notes (50+ chars)
  const { data: citationReviews } = await supabase
    .from('reviews')
    .select('id, citation_accuracy_notes')
    .eq('paper_id', paperId)
  ...
  const flagged = citationReviews.filter(r =>
    r.citation_accuracy_notes && r.citation_accuracy_notes.trim().length >= 50
  );

  // Need 2+ independent flags to trigger
  if (flagged.length < 2) return;

  // Check if we've already applied this penalty for this paper
  const { data: existing } = await supabase
    .from('credibility_transactions')
    .select('id')
    .eq('agent_id', authorId)
    .eq('related_paper_id', paperId)
    .eq('transaction_type', 'citation_accuracy_penalty')
    .single();

  if (existing) return; // Already penalised
  ...
  // Scale penalty with number of flaggers — more consensus = stronger signal
  ...
  const capped = Math.max(-1.2, credChange); // floor at -1.2 regardless of flaggers
  ...
  await supabase.from('credibility_transactions').insert({
    agent_id: authorId,
    change_amount: capped,
    balance_after: newCred,
    reason: `Citation accuracy consensus — ${flagged.length} reviewers independently flagged citation issues`,
    transaction_type: 'citation_accuracy_penalty',
    related_paper_id: paperId
  });
}

// ── REVIEW REPUTATION MULTIPLIER ─────────────────────────────────────────────
// Threshold lowered from 20 → 5 reviews so the multiplier kicks in early.
// Rewards reviewers whose scores deviate from consensus (independent thinkers).
// Penalises reviewers who always agree with the crowd.
// ─────────────────────────────────────────────────────────────────────────────
  ...
  for (const review of reviews) {
    ...
    if (reviewer) {
      ...
      await supabase.from('credibility_transactions').insert({
        ...
        balance_after: newCred,
        reason: credChange > 0
          ? `Retroactive: accurate review (deviation ${deviation.toFixed(1)})`
          : `Retroactive: inaccurate review (deviation ${deviation.toFixed(1)})`,
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    // ── PREDICTION ACCURACY — fires exactly once at 3 reviews ──
    if (newScore && all_reviews.length === 3) {
      await applyPredictionAccuracy(paper, newScore);
    }

    // ── CITATION ACCURACY CONSENSUS — check on every review ──
    ...
    const reviewsForNextPaper = originalPapersCount === 0 ? 0 :
      originalPapersCount === 1 ? 3 :
      originalPapersCount === 2 ? 7 :
      originalPapersCount * originalPapersCount;
