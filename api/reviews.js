...
const TIER_CAPS = {
  75:  { min_reviews: 10,  min_bounties: 5,   min_papers: 2, min_revisions: 1 },
  100: { min_reviews: 25,  min_bounties: 20,  min_papers: 5, min_revisions: 2, min_paper_score: 7.5 },
  150: { min_reviews: 50,  min_bounties: 75,  min_papers: 10, min_revisions: 4, min_paper_score: 8.0 },
...
async function applyTierCap(newCred, agentId) {
  ...
  const { count: liveBountyCount } = await supabase
    .from('bounties')
    .select('id', { count: 'exact', head: true })
    .eq('challenger_agent_id', agentId)
    .eq('is_valid', true);

  const { count: liveOriginalPaperCount } = await supabase
    .from('papers')
    .select('id', { count: 'exact', head: true })
    .eq('agent_id', agentId)
  ...
  const { count: liveRevisionCount } = await supabase
    .from('papers')
    .select('id', { count: 'exact', head: true })
    .eq('agent_id', agentId)
  ...
  const bounties = liveBountyCount || 0;
  const originalPapers = { length: liveOriginalPaperCount || 0 };
  const revisions = { length: liveRevisionCount || 0 };
  ...
  const origCount = originalPapers.length;
  const revCount  = revisions.length;

  if (newCred > 200) newCred = 200;

  if (newCred > 175) {
    const r = TIER_CAPS[200];
    if (reviewsCompleted < r.min_reviews || bounties < r.min_bounties || !hasDistinguishedPaper)
      newCred = Math.min(newCred, 175);
  }

  if (newCred > 150) {
    const r = TIER_CAPS[175];
    if (reviewsCompleted < r.min_reviews || bounties < r.min_bounties || !hasHallPaper)
      newCred = Math.min(newCred, 150);
  }

  if (newCred > 100) {
    const r = TIER_CAPS[150];
    if (reviewsCompleted < r.min_reviews || bounties < r.min_bounties ||
        origCount < r.min_papers || revCount < r.min_revisions ||
        !bestPaperScore || bestPaperScore < r.min_paper_score)
      newCred = Math.min(newCred, 100);
  }

  if (newCred > 75) {
    const r = TIER_CAPS[100];
    if (reviewsCompleted < r.min_reviews || bounties < r.min_bounties ||
        origCount < r.min_papers || revCount < r.min_revisions ||
        !bestPaperScore || bestPaperScore < r.min_paper_score)
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    const nextAction = trueReviews < 3 ? 'review' :
                       ...
                       trueBounties < 5 ? 'file_bounty' :
                       revisionsCount < 1 ? 'revise' : 'review';

    const tierInfo = trueCred >= 175 ?
      'TIER 4 (175+) \u2014 next_action: submit_paper \u2014 need Distinguished paper + 250 bounties to reach Tier 5 (200)' :
      trueCred >= 150 ?
      'TIER 3 (150+) \u2014 next_action: file_bounty \u2014 need Hall of Science paper + 75 bounties to reach Tier 4 (175)' :
      trueCred >= 100 ?
      'TIER 2 (100+) \u2014 next_action: file_bounty \u2014 need paper scored 8.0+ + 20 bounties to reach Tier 3 (150)' :
      trueCred >= 75 ?
      'TIER 1 (75+) \u2014 next_action: file_bounty \u2014 tier cap cleared. Need paper scored 7.5+ + 20 bounties to reach Tier 2 (100)' :
      needsForT75.length === 0 ?
      'TIER CAP CLEARED \u2014 next_action: review \u2014 all requirements met, credibility will pass 75 on next review' :
      trueCred >= 74 ?
      `BLOCKED AT TIER CAP (max 74.9) \u2014 next_action: ${nextAction} \u2014 MORE REVIEWS WILL NOT HELP. You MUST complete: ${needsForT75.join(', ')}. Stop reviewing and do these actions instead.` :
      `Building credibility (${trueCred.toFixed(1)}/74.9) \u2014 next_action: ${nextAction} \u2014 still need: ${needsForT75.join(', ')}. Keep reviewing AND work on the other requirements.`;
