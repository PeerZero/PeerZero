...
const TIER_CAPS = {
  75:  { min_reviews: 10,  min_bounties: 5 },
  100: { min_reviews: 10,  min_bounties: 5 },
  ...
  if (req.method === 'POST') {
    ...
    if (paper.parent_paper_id && paper.response_stance !== 'revision' && newScore && all_reviews.length >= 3) {
      ...
      if (paper.response_stance === 'rebut') {
        // Strong rebuttal (low score on rebuttal = reviewer agrees with challenge) pulls parent down
        impact = -(deviation * 0.3); // e.g. rebuttal scores 3 → impact = -(3-5.5)*0.3 = +0.75 negative
      } else if (paper.response_stance === 'support') {
        impact = deviation * 0.2;
      }
    ...
    const tierInfo = trueCred >= 175 ?
      'Elite — need Distinguished paper + 250 bounties for Legendary' :
      trueCred >= 150 ?
      'Senior — need Hall of Science paper + 75 bounties for Elite' :
      trueCred >= 100 ?
      'Established — need 8.0+ paper + 20 bounties for Senior' :
      trueCred >= 75 ?
      `Tier 75 unlocked — need 25 reviews + 20 bounties + paper scored 8.0+ to unlock 150` :
      `Newcomer — ${trueReviews} reviews done, need ${Math.max(0, 10 - trueReviews)} more reviews + ${Math.max(0, 5 - trueBounties)} more bounties to unlock 75`;
