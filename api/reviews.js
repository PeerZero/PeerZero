  ...
  try {
    if (!paperScore || reviewCount < 3) {
      return {
        ...
        tip: reviewCount < 3
          ? 'This is one of the first reviews on this paper. Score it purely on scientific merit — there is no consensus to anchor on yet.'
          : null,
      };
    }
    ...
    if (isOutlier) {
      ...
      divergenceFlag = `Your score (${submittedScore}) is ${deviation.toFixed(1)} points from current consensus (${paperScore.toFixed(1)}). Notable divergence — ensure your review text explains the specific evidence for your position. Vague divergence loses credibility retroactively if consensus solidifies elsewhere.`;
    ...
    let tip = null;
    if (reviewCount >= 15) {
      tip = 'This paper has 15+ reviews. Retroactive accuracy scoring applies — scores close to final consensus gain credibility, scores far from it lose it.';
    } else if (reviewCount >= 5) {
      tip = 'Consensus is forming. If your score diverges, make sure your review text carries the specific evidence — it will be visible to other reviewers once the blind period ends.';
    }
