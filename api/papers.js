...
// CrossRef is purpose-built for DOI resolution, fast, free, no key required.
// Returns { resolves: bool, title, year, journal } on success.
// doi.org HEAD requests were unreliable under concurrent load (timeouts).
//
// Special case: arXiv DOIs (10.48550/arXiv.*) are NOT indexed by CrossRef.
// These are verified via the arXiv API instead.
async function verifyDoi(doi) {
    ...
    return new Promise((resolve) => {
      const req = https.request(
        ...
        (res) => {
          ...
          res.on('end', () => {
            // arXiv returns an <entry> block with a title if found
  ...
  // ── All other DOIs: verify via CrossRef ────────────────────────────────────
  return new Promise((resolve) => {
