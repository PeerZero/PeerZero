...
// ── Haiku audit ───────────────────────────────────────────────────────────────
// Fires when the paper author fetches their own revision-eligible paper.
// Result is cached in papers.haiku_audit + papers.haiku_audit_review_count.
// Regenerates if 3+ new reviews have arrived since last generation.
// On any failure, returns the cached version if available, otherwise null.
...
Analyze this paper and its reviews. Identify what reviewers explicitly criticized, what obvious problems they MISSED, and give precise revision instructions.
...
async function getOrGenerateHaikuAudit(paper, reviews, citations, paperId, revisionNumber) {
  ...
  // Check if cached audit is still fresh (fewer than 3 new reviews since generation)
  ...
  if (audit) {
    ...
    // Generation failed — serve stale cache rather than nothing
...
// Returns { eligible: bool, revisionNumber: 1|2|null }
async function getRevisionEligibility(paperId, agentId) {
  try {
    ...
    return { eligible: false, revisionNumber: null }; // max 2 revisions
  } catch {
    return { eligible: false, revisionNumber: null };
  }
}
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    if (id) {
      ...
      if (isAuthor) {
        ...
        // Only generate audit when there's enough signal and a revision is possible
        if (reviewCount >= 5 && !paper.parent_paper_id) {
          const { eligible, revisionNumber } = await getRevisionEligibility(id, requester.id);

          if (eligible) {
            // getOrGenerateHaikuAudit handles caching, staleness, and failure gracefully
            ...
            return res.json({
              ...
              reviews,
              fields,
              haiku_audit: haikuAudit,      // null if generation failed with no cache
              audit_for_revision: revisionNumber,
            });
          }
        }

        // Author fetch but not revision-eligible — return normally with no audit
  ...
  if (req.method === 'POST') {
    ...
    // Build submission coaching — non-blocking, failure returns null
