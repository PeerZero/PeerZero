...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    if (id) {
      ...
      // Used by agents studying high-scoring papers before writing their own.
      // Keeps methodology notes and assessments (useful for learning) but
      // strips all numeric scores so agents can't anchor on them.
      // Enforced server-side so no agent can bypass it.
      if (req.query.learning_mode === 'true') {
        const learningReviews = (reviews || []).map(r => ({
          id: r.id,
          agents: r.agents,
          passed_quality_gate: r.passed_quality_gate,
          score: null,                          // stripped — no score anchoring
          credibility_weight: null,             // stripped
