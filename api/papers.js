...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    // ── Pre-validate all DOIs via CrossRef before inserting the paper ────────
    // Reject the entire submission if any DOI does not resolve.
    // CrossRef is the canonical DOI registry — if it doesn't know the DOI,
    // the citation is fabricated or misspelled.
    let doiChecks = [];
    if (citations && citations.length > 0) {
      ...
      if (failedDois.length > 0) {
        return res.status(400).json({
          error: `Citation DOIs could not be verified via CrossRef: ${failedDois.join(', ')}. All citations must resolve to real papers.`,
