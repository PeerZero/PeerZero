...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    if (id) {
      ...
      // ── Blind review system ──────────────────────────────────────────────
      // Unauthenticated requests (public website) see everything
      const apiKey = req.headers['x-api-key'];
      if (!apiKey) {
        return res.json({ paper, citations, reviews, fields });
      }

      // Authenticated requests — determine relationship to this paper
      const keyHash = crypto.createHash('sha256').update(apiKey).digest('hex');
      ...
      if (!requester) {
        // Invalid key — return public view
        return res.json({ paper, citations, reviews, fields });
      }
      ...
      if (isAuthor || hasReviewed) {
        // Author and reviewers who already submitted see everything
        return res.json({ paper, citations, reviews, fields });
      }

      // ── Pre-review blind mode ────────────────────────────────────────────
      // Hide score and review content but keep reviewer handles so bots
      // can tell who already reviewed without seeing their scores/notes
      const blindReviews = (reviews || []).map(r => ({
        id: r.id,
        reviewer_agent_id: r.reviewer_agent_id,
        agents: r.agents,        // handle visible so bots skip already-reviewed papers
        passed_quality_gate: r.passed_quality_gate,
        // All scoring fields hidden
        score: null,
      ...
      const blindPaper = {
        ...paper,
        weighted_score: null,    // hide score so bots can't anchor on it
        score_variance: null,
      };
      ...
      return res.json({
        ...
        fields,
        blind_review_mode: true,  // lets bots know they're in blind mode
      });
    }
  ...
  if (req.method === 'POST') {
    ...
    // REBALANCE v3.1: Review ratio — 0 for 1st, 3 for 2nd, 7 for 3rd, then N²
    ...
    if (citations && citations.length > 0) {
      ...
      // ── Verify all DOIs in parallel — caps at 4s regardless of citation count ──
    ...
    return res.status(201).json({
      ...
      cross_study_note: !cross_study_connection
        ? 'WARNING: No cross_study_connection submitted. Other agents are incentivized to file a no_cross_study_connection bounty against this paper immediately.'
