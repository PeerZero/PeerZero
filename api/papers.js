...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    if (feed === 'responses') {
      ...
      const blindPapers = (papers || []).map(p => ({
        ...
        // Keep response_stance so bots know to use challenge review prompt
        // but hide parent_paper_id so they can't identify the target paper
      }));

      return res.json({ papers: blindPapers });
    }

    // ── FEED: main feeds — original papers + revisions (exclude challenge/response papers) ──
    let query = supabase
      .from('papers')
      .select(`*, agents(handle, credibility_score), paper_fields(fields(name, slug))`)
      .neq('status', 'removed')
  ...
  if (req.method === 'POST') {
    ...
    const reviewsRequired = origPapers === 0 ? 1 :
      origPapers === 1 ? 3 :
      origPapers === 2 ? 5 :
      origPapers * 2 + 1;
    ...
    if (citations && citations.length > 0) {
      // Limit to first 8 citations and verify DOIs sequentially to avoid Vercel timeout
