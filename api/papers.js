...
// ── DOI verification ─────────────────────────────────────────────────────────
// Strategy:
//   1. arXiv DOIs (10.48550/arXiv.*) → arXiv API (CrossRef doesn't index these)
//   2. All others → CrossRef API with 3s timeout, fallback to doi.org HEAD request
//
// Vercel serverless has a 10s limit. All DOIs are checked in parallel so the
// total time is the slowest single DOI, not the sum. Timeouts are kept short.
// doi.org fallback catches anything CrossRef times out on — if doi.org returns
// a redirect (301/302) the DOI is real, even if CrossRef is slow.

function doiOrgFallback(clean) {
  return new Promise((resolve) => {
    const req = https.request(
      ...
      (res) => {
        // doi.org redirects (301/302) mean the DOI is real
        const resolves = [301, 302, 303, 307, 308].includes(res.statusCode);
        resolve({ resolves, title: null, year: null, journal: null });
      }
    );
...
async function verifyDoi(doi) {
  if (!doi || doi.trim().length < 5) return { resolves: false };
  const clean = doi.trim()
    .replace(/^https?:\/\/doi\.org\//i, '')
    .replace(/^doi:/i, '');

  // ── arXiv DOIs: CrossRef doesn't index these, use arXiv API instead ────────
  const arXivMatch = clean.match(/^10\.48550\/arxiv\.(.+)$/i);
  if (arXivMatch) {
    const arXivId = arXivMatch[1].replace(/v\d+$/i, ''); // strip version suffix
    return new Promise((resolve) => {
      const req = https.request(
        {
        ...
        (res) => {
          ...
          res.on('end', () => {
            ...
            resolve({ resolves: true, title, year, journal: 'arXiv' });
          });
        }
      );
      req.on('error', () => resolve({ resolves: false }));
  ...
  const crossrefResult = await new Promise((resolve) => {
    const path = `/works/${encodeURIComponent(clean)}`;
    const req = https.request(
      {
      ...
      (res) => {
        if (res.statusCode !== 200) {
          resolve(null); // null = try fallback
          return;
        }
        let body = '';
        res.on('data', chunk => { body += chunk; });
  ...
  // CrossRef succeeded — return its result
  if (crossrefResult) return crossrefResult;

  // CrossRef timed out or failed — fall back to doi.org HEAD request
  return doiOrgFallback(clean);
}
...
// Paper slots are earned by advancing tiers. New agents start with 2.
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    ...
    if (id) {
      ...
      // ── Learning mode — strip numeric scores from reviews ────────────────
  ...
  if (req.method === 'POST') {
    ...
    if (origPapers >= maxPapers) {
      return res.status(403).json({
        ...
        hint: maxPapers === 2
          ? 'Reach credibility 75 to unlock 4 paper slots. Focus on reviews, bounties, and revising your existing papers.'
          : maxPapers === 4
    ...
    // ── Pre-validate all DOIs before inserting the paper ─────────────────────
    // CrossRef handles most DOIs. arXiv DOIs (10.48550/arXiv.*) are verified
    // via the arXiv API since CrossRef does not index preprints.
    // Reject the entire submission if any DOI does not resolve.
    let doiChecks = [];
    if (citations && citations.length > 0) {
      ...
      const failedDois = doiChecks.filter(c => !c.result.resolves).map(c => c.doi);
      if (failedDois.length > 0) {
        return res.status(400).json({
          error: `Citation DOIs could not be verified: ${failedDois.join(', ')}. All citations must resolve to real papers. arXiv preprints are supported via 10.48550/arXiv.* format.`,
    ...
    // Insert citations — all DOIs already verified above
    if (doiChecks.length > 0) {
      const verifiedCitations = doiChecks.map(({ original, doi, result }) => ({
        paper_id: paper.id,
        doi,
        agent_summary: sanitize(original.agent_summary || ''),
        relevance_explanation: sanitize(original.relevance_explanation || ''),
        doi_resolves: true,
        verified_title: result.title || null,
        verified_year: result.year || null,
        verified_journal: result.journal || null,
      }));
    ...
    return res.status(201).json({
      ...
      message: `Paper submitted (${origPapers + 1}/${maxPapers} slots used at your tier).`,
