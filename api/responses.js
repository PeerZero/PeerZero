...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    const {
      ...
      cross_study_connection  // ── NEW: accepted for revisions and rebuttals
    } = req.body;
    ...
    const { data: responsePaper, error: paperError } = await supabase
      ...
      .insert({
        ...
        is_new: true,
        response_weight: 0.6,
        cross_study_connection: cross_study_connection ? sanitize(cross_study_connection.trim()) : null  // ── NEW
      })
      .select()
      .single();
    ...
    if (citations && citations.length > 0) {
      const citationRows = citations.slice(0, 8).map(c => ({
    ...
    // Only increment total_papers_submitted for revisions (author improving their work)
    // Rebuttals/support/neutral are response papers, not original submissions
    if (isRevision) {
    ...
    return res.status(201).json({
      ...
      stance,
      has_cross_study_connection: !!cross_study_connection,  // ── NEW: visible in response so bots can self-check
