...
// ── DOI verification (mirrors papers.js exactly) ──────────────────────────────
async function verifyDoi(doi) {
  if (!doi || typeof doi !== 'string') return { resolves: false };

  const normalized = doi.trim().replace(/^https?:\/\/doi\.org\//i, '');

  // arXiv DOIs — verify via arXiv API, not CrossRef
  if (/^10\.48550\/arxiv\./i.test(normalized)) {
    const arxivId = normalized.replace(/^10\.48550\/arxiv\./i, '');
    try {
      const result = await new Promise((resolve) => {
        const url = `https://export.arxiv.org/abs/${arxivId}`;
        const req = https.get(url, { timeout: 5000 }, (res) => {
          resolve({ resolves: res.statusCode === 200, journal: 'arXiv' });
        });
  ...
  try {
        ...
        if (res.statusCode === 200) {
          ...
          res.on('end', () => {
            try {
              ...
              const journal =
                parsed?.message?.['container-title']?.[0] ||
                parsed?.message?.publisher ||
                'Unknown Journal';
              resolve({ resolves: true, journal });
            } catch {
              resolve({ resolves: true, journal: 'Unknown Journal' });
            }
          });
        } else if (res.statusCode === 404) {
          resolve({ resolves: false });
        } else {
          resolve(null); // trigger fallback
        }
      });
    ...
    // CrossRef timed out or returned non-200/404 — fall back to doi.org HEAD
    const doiOrgResult = await new Promise((resolve) => {
      const req = https.request(
        `https://doi.org/${encodeURIComponent(normalized)}`,
        { method: 'HEAD', timeout: 4000 },
        (res) => {
          resolve({ resolves: [301, 302, 303].includes(res.statusCode), journal: 'Verified via doi.org' });
        }
      );
      req.on('error', () => resolve({ resolves: false }));
      req.on('timeout', () => { req.destroy(); resolve({ resolves: false }); });
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    const {
      title, abstract, body, stance, citations,
    ...
    if (!stance || !['support', 'neutral', 'rebut', 'revision'].includes(stance)) return res.status(400).json({ error: 'Stance must be support, neutral, rebut, or revision' });
    ...
    if (isRevision) {
      ...
      if (revisionCount >= 2) {
        return res.status(409).json({ error: 'Maximum of 2 revisions allowed per paper' });
      }
    ...
    // ── Verify DOIs before inserting citations ────────────────────────────────
    if (citations && citations.length > 0) {
      const sliced = citations.slice(0, 8);

      const doiChecks = await Promise.all(
        sliced.map(async (c) => {
          const result = await verifyDoi(c.doi);
          return { citation: c, result };
        })
      );

      const citationRows = doiChecks.map(({ citation, result }) => ({
        paper_id: responsePaper.id,
        doi: citation.doi ? String(citation.doi).slice(0, 200) : '',
