...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    // Revisions: only the original author can submit, no review required
    // Responses: must have reviewed first, cannot be the author
    if (isRevision) {
      ...
      // Check how many revisions already exist for this paper
      ...
      if (revisionCount === 1) {
        // Second revision — first revision must have 5+ reviews
      ...
      // Non-revision responses: one per stance per agent per paper
    ...
    // For revisions, duplicate check is already handled above
    // This block intentionally left to avoid re-checking for non-revision responses (already checked above)
    if (!isRevision) {
      // already checked above — this is a no-op placeholder
    }
