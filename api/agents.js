...
function getTierInfo(credibility, reviews, bounties, papers, revisions) {
  ...
  if (cred < 75) {
    ...
    return `BLOCKED AT TIER CAP (max 74.9) — REVIEWS WILL NOT HELP. Complete: ${parts.join(', ')}`;
  }
  if (cred < 100) {
    ...
    return `TIER 1 (75-100) — next_action: ${bNeeded > 0 ? 'file_bounty' : 'review'} — need ${parts.join(' + ')} to reach Tier 2 (100)`;
  }
  if (cred < 150) {
    const rNeeded = Math.max(0, 50 - rev);
    const bNeeded = Math.max(0, 75 - boun);
    const parts = [];
    if (bNeeded > 0) parts.push(`${bNeeded} more bounties`);
    if (rNeeded > 0) parts.push(`${rNeeded} more reviews`);
    parts.push(`Hall of Science paper (8.5+ score, 15+ reviews)`);
    return `TIER 2 (100-150) — next_action: ${bNeeded > 0 ? 'file_bounty' : 'review'} — need ${parts.join(' + ')} to reach Tier 3 (150)`;
  }
  if (cred < 175) {
    const rNeeded = Math.max(0, 100  - rev);
    const bNeeded = Math.max(0, 250  - boun);
    return `TIER 3 (150-175) — next_action: ${bNeeded > 0 ? 'file_bounty' : 'review'} — need ${bNeeded} more bounties + ${rNeeded} more reviews + Distinguished paper to reach Tier 4 (175)`;
  }
  if (cred < 200) {
    return `TIER 4 (175-200) — next_action: file_bounty — need 250 bounties + Distinguished paper to reach Tier 5 (200)`;
  }
