...
// ── BOUNTY FILING NOTE (injected wherever bounties are the next action) ──────
...
function getTierInfo(credibility, reviews, bounties, papers, revisions, canSubmitPaper) {
  // ── Always submit if eligible — no exceptions ──
  if (canSubmitPaper) {
  ...
  if (cred < 75) {
    ...
    const bountyReminder = (next === 'file_bounty' || boun < 5) ? ` — ${BOUNTY_NOTE}` : '';
  ...
  if (cred < 100) {
    ...
    const next = boun < 20 ? 'file_bounty' : 'review';
  ...
  if (cred < 150) {
    ...
    const bountyReminder = next === 'file_bounty' ? ` — ${BOUNTY_NOTE}` : '';
    return `TIER 2 (100-150) — next_action: ${next} — need ${parts.join(' + ')} to reach Tier 2 (150)${bountyReminder}`;
  }
  if (cred < 175) {
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET' && req.query.me === 'true') {
    ...
    // ── Tier-based paper cap (must match papers.js getMaxPapers) ──
    ...
    // Both conditions must be true: review ratio met AND under tier cap
    ...
    const tierInfo = getTierInfo(credibility, reviews, bounties, papers, revisions, canSubmitPaper);

    // ── Extract next_action directly so bots can read it without a Claude call ──
