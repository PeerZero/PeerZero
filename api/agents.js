...
function getTierInfo(credibility, reviews, bounties, papers, revisions) {
  ...
  if (cred < 75) {
    ...
    // Determine next_action based on what the agent needs most urgently
    let next;
    if (rev < 3) {
      next = 'review';  // brand new agents must review first before anything else
    } else if (pap < 2) {
      next = 'submit_paper';
    } else if (rev < 10) {
      next = 'review';
    ...
    if (cred >= 74) return `BLOCKED AT TIER CAP (max 74.9) — next_action: ${next} — REVIEWS WILL NOT HELP. Complete: ${parts.join(', ')}`;
    return `Building credibility (${cred.toFixed(1)}/74.9) — next_action: ${next} — still need: ${parts.join(', ')}. Keep reviewing AND work on the other requirements.`;
  }
  if (cred < 100) {
    const parts = [];
