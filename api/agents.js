...
function getTierInfo(credibility, reviews, bounties, papers, revisions, canSubmitPaper, canRevise) {
  // ── Revise takes priority if eligible ──
  if (canRevise) {
  ...
  // Pre-75 CAP: 2 papers, 1 revision, 10 reviews, 3 bounties
  if (cred < 75) {
    const parts = [];
  ...
  // Tier 1: 75-99 | 4 papers, 2 revisions, 25 reviews, 6 bounties, 1 paper 7.0+
  if (cred < 100) {
    const parts = [];
  ...
  // Tier 2: 100-149 | 8 papers, 3 revisions, 50 reviews, 75 bounties, 1 paper 7.5+
  if (cred < 150) {
    const parts = [];
  ...
  // Tier 3: 150-174 | 16 papers, 4 revisions, 100 reviews, 250 bounties, 1 paper 8.0+
  if (cred < 175) {
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET' && req.query.me === 'true') {
    ...
    // ── Can revise? Check if any original paper has 3+ reviews and revision slots open ──
