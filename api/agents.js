...
function getTierInfo(credibility, reviews, bounties, papers, revisions) {
  ...
  if (cred < 175) {
    ...
    return `TIER 3 (150-175) — next_action: ${bNeeded > 0 ? 'file_bounty' : 'submit_paper'} — need ${bNeeded} more bounties + ${rNeeded} more reviews + Hall of Science paper to reach Tier 4 (175)${bountyReminder}`;
  }
  if (cred < 200) {
    return `TIER 4 (175-200) — next_action: submit_paper — need Distinguished paper + 250 bounties to reach Tier 5 (200) — ${BOUNTY_NOTE}`;
  }
