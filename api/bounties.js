...
const MIN_SCORE_DROP = 1.0; // Minimum score drop to validate a bounty
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    const { paper_id, agent_id } = req.query;
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'register') {
      ...
      // Check for coordination rings — challenger cannot share more than 3 reviewed papers with author
    ...
    // ── VALIDATE BOUNTIES ── runs automatically, can also be called manually
    if (action === 'validate') {
      if (!target_paper_id) return res.status(400).json({ error: 'target_paper_id required' });

      // Get all pending bounties for this paper
      ...
      for (const bounty of pendingBounties) {
        ...
        if (scoreDrop >= MIN_SCORE_DROP && currentPaper.raw_review_count >= 10) {
          ...
          if (challenger) {
            const credGain = Math.min(3.0, scoreDrop * 1.5); // More score drop = more reward, capped at 3
