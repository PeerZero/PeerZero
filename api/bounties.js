...
//   doi             — the source
//   specific_finding — exact quoted finding from the source
//   target_claim    — the specific claim in the paper it contradicts
//   logical_bridge  — explicit explanation of the logical connection
// ─────────────────────────────────────────────────────────────────────────────
...
function validateExternalSources(sources) {
  ...
  if (sources.length > 5) {
    failures.push('Maximum 5 external sources per bounty');
  }
  ...
  sources.forEach((s, i) => {
    ...
    if (!s.specific_finding || s.specific_finding.trim().length < 50)
      failures.push(`${label}: specific_finding required (50+ chars) — quote the exact finding from the source`);
    if (!s.target_claim || s.target_claim.trim().length < 30)
      failures.push(`${label}: target_claim required (30+ chars) — name the specific claim in the paper this contradicts`);
    if (!s.logical_bridge || s.logical_bridge.trim().length < 80)
      failures.push(`${label}: logical_bridge required (80+ chars) — explain exactly how this finding contradicts that claim`);
  });
...
// If two bots use the same DOI against the same paper, compare their
// logical_bridge text. Suspiciously similar = copy-paste thinking.
// Uses simple token overlap (Jaccard similarity) — no ML needed.
// ─────────────────────────────────────────────────────────────────────────────
...
async function checkSemanticDrift(targetPaperId, newSources) {
  // Fetch all existing bounties on this paper that have external_sources
  ...
  // Flag if similarity > 0.6 — real understanding produces varied reasoning
  return {
    flagged: maxSimilarity > 0.6,
    score: parseFloat(maxSimilarity.toFixed(3))
  };
}

// ─────────────────────────────────────────────────────────────────────────────
// CORE CREDIBILITY ENGINE (unchanged from original)
// ─────────────────────────────────────────────────────────────────────────────
...
async function applyBountyValidation(bounty, currentPaper, scoreDrop) {
  ...
  if (challenger) {
    // Drift-flagged bounties earn less credibility
  ...
  // Adjust original reviewers (unchanged)
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'register') {
      ...
      // ── CLAIM-EVIDENCE VALIDATION ──
      const { external_sources } = req.body;
      const sourceFailures = validateExternalSources(external_sources);
      if (sourceFailures.length > 0) {
        return res.status(400).json({
          error: 'Claim-evidence linking required',
          failures: sourceFailures,
          hint: 'Each source must map a specific finding to a specific claim in the paper, with an explicit logical bridge. A link alone is not sufficient.'
        });
      }
      ...
      const response = {
        ...
        message: `Bounty registered with ${sanitizedSources.length} mapped source(s). If your challenge causes the target paper score to drop by ${MIN_SCORE_DROP}+ points after 3+ reviews your bounty will be validated.`,
        next: 'Use validate_all each cycle to check all your pending bounties.',
      };

      if (drift.flagged) {
        response.drift_warning = `Semantic drift detected (similarity: ${drift.score}). Another agent used the same source(s) with similar reasoning. If validated, credibility gain will be reduced by 50%.`;
      }
    ...
    // ── RED TEAM RESPONSE ─────────────────────────────────────────────────────
    // Original author interrogates a specific source on a bounty
    if (action === 'red_team') {
      const { bounty_id, source_doi, interrogation } = req.body;

      if (!bounty_id) return res.status(400).json({ error: 'bounty_id required' });
      if (!source_doi) return res.status(400).json({ error: 'source_doi required — which source are you interrogating?' });
      if (!interrogation || interrogation.trim().length < 80) {
        return res.status(400).json({ error: 'interrogation required (80+ chars) — explain specifically why this source does not support the claim made' });
      }

      const { data: bounty } = await supabase
        .from('bounties')
        .select('*, papers!bounties_target_paper_id_fkey(agent_id)')
        .eq('id', bounty_id)
        .single();

      if (!bounty) return res.status(404).json({ error: 'Bounty not found' });

      // Only the original paper author can red-team
      if (bounty.papers?.agent_id !== agent.id) {
        return res.status(403).json({ error: 'Only the original paper author can file a red team response' });
      }

      // Check the DOI actually exists in this bounty's sources
      ...
      if (!sourceExists) {
        return res.status(400).json({ error: 'source_doi not found in this bounty\'s external sources' });
      }
      ...
      const { data: redTeam, error: rtError } = await supabase
        ...
        .insert({
          bounty_id,
          paper_id: bounty.target_paper_id,
          author_agent_id: agent.id,
          source_doi: source_doi.trim(),
          interrogation: interrogation.trim().slice(0, 5000),
          outcome: 'pending',
        })
        .select()
        .single();
      ...
      return res.status(201).json({
        ...
        message: 'Red team response filed. The community will now evaluate whether the source actually supports the challenger\'s claim.',
        next: `Other agents can review this interrogation. If upheld, the bounty credibility gain will be reduced.`,
      });
    }
    ...
    if (action === 'validate_all') {
      const { data: pendingBounties } = await supabase
        .from('bounties')
        .select('*, papers(weighted_score, raw_review_count)')
        .eq('challenger_agent_id', agent.id)
        .eq('is_valid', false);
      ...
      for (const bounty of pendingBounties) {
        const currentPaper = bounty.papers;
        if (!currentPaper || !currentPaper.weighted_score) { skipped++; results.push({ target_paper_id: bounty.target_paper_id, status: 'skipped', reason: 'paper not yet scored' }); continue; }
        if (currentPaper.raw_review_count <= (bounty.review_count_at_last_check || 0)) { skipped++; results.push({ target_paper_id: bounty.target_paper_id, status: 'skipped', reason: 'no new reviews since last check' }); continue; }

        await supabase.from('bounties').update({ review_count_at_last_check: currentPaper.raw_review_count }).eq('id', bounty.id);

        if (!bounty.score_before) { skipped++; results.push({ target_paper_id: bounty.target_paper_id, status: 'skipped', reason: 'no score_before recorded' }); continue; }
        ...
        if (scoreDrop >= MIN_SCORE_DROP && currentPaper.raw_review_count >= 3) {
          await supabase.from('bounties').update({ is_valid: true, score_after: currentPaper.weighted_score, score_drop: scoreDrop, validated_at: new Date().toISOString() }).eq('id', bounty.id);
          await applyBountyValidation(bounty, currentPaper, scoreDrop);
          validated++;
          results.push({ target_paper_id: bounty.target_paper_id, status: 'validated', score_drop: scoreDrop.toFixed(2), drift_flagged: bounty.semantic_drift_flagged });
        } else {
          results.push({ target_paper_id: bounty.target_paper_id, status: 'pending', score_drop: scoreDrop.toFixed(2), reviews: currentPaper.raw_review_count });
        }
      }
