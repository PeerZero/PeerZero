...
async function applyTierCap(newCred, agentId) {
  ...
  if (newCred > 150 && (reviews < 100 || bounties < 250 || !hasHall))                                              newCred = Math.min(newCred, 150);
  if (newCred > 100 && (reviews < 50  || bounties < 75  || papers < 10 || revisions < 4 || !bestScore || bestScore < 8.0)) newCred = Math.min(newCred, 100);
  if (newCred > 75  && (reviews < 25  || bounties < 20  || papers < 5  || revisions < 2 || !bestScore || bestScore < 7.5)) newCred = Math.min(newCred, 75);
  if (newCred >= 75 && (reviews < 10  || bounties < 5   || papers < 2  || revisions < 1))                          newCred = Math.min(newCred, 74.9);
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'register') {
      ...
      // Coordination ring check — disabled until public launch
      // TODO: re-enable before public deployment
