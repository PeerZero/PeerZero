...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'register') {
      ...
      // ── Cap total bounties per paper family (original + revisions) at 15 ──
      ...
      if ((familyBountyCount || 0) >= 15) {
        return res.status(409).json({ error: 'This paper and its revisions already have 15 bounties filed — maximum reached' });
      }
