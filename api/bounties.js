...
// REBALANCE v3: lowered from 1.0 to 0.8, then to 0.2 for testing
...
module.exports = async (req, res) => {
  ...
  // POST — register a challenge, validate a single paper, or validate all at once
  if (req.method === 'POST') {
    ...
    if (!action) return res.status(400).json({ error: 'action must be register, validate, or validate_all' });
    ...
    if (action === 'register') {
      ...
      // ── REBALANCE v3: Cap total bounties per paper family at 8 ──
      ...
      return res.status(201).json({
        ...
        next: 'Use validate_all each cycle to check all your pending bounties in one call instead of one call per paper.'
      });
    }

    // ── VALIDATE ALL PENDING BOUNTIES (one call instead of N calls) ──
    // Skips papers where raw_review_count hasn't changed since last check — no new reviews = no score change possible
    if (action === 'validate_all') {
      ...
      if (!pendingBounties || pendingBounties.length === 0) {
        return res.json({
          success: true,
          message: 'No pending bounties to validate',
          bounties_checked: 0,
          bounties_validated: 0,
          bounties_skipped: 0
        });
      }
      ...
      for (const bounty of pendingBounties) {
        ...
        // Skip if paper has no score yet
        ...
        // ── KEY OPTIMISATION: skip if no new reviews since last check ──
        // No new reviews = score cannot have changed = nothing to validate
        if (currentPaper.raw_review_count <= (bounty.review_count_at_last_check || 0)) {
          skipped++;
          results.push({ target_paper_id: bounty.target_paper_id, status: 'skipped', reason: 'no new reviews since last check' });
          continue;
        }

        // Update the last-check count regardless of outcome so next cycle skips correctly
        await supabase.from('bounties')
          .update({ review_count_at_last_check: currentPaper.raw_review_count })
          .eq('id', bounty.id);
    ...
    // ── VALIDATE SINGLE PAPER (kept for backward compatibility) ──
    if (action === 'validate') {
      if (!target_paper_id) return res.status(400).json({ error: 'target_paper_id required' });
    ...
    return res.status(400).json({ error: 'action must be register, validate, or validate_all' });
  }
