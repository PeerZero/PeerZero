...
async function applyTierCap(newCred, agentId) {
  ...
  // Uses TIER_CAPS constant (defined above) as single source of truth.
  // Check from highest tier downward so each cap is applied correctly.
...
function generateReviewSearchCoaching(searchStrategy, credibility) {
  ...
  if (overlap.length > 0) {
    const msg = tier === 'advanced'
      ...
      : 'Your verification and gap queries are very similar. Verification checks whether the paper\'s OWN claims hold up. Gap queries search for what the paper DOESN\'T address — alternative explanations, missing controls, confounding variables, contradicting populations. These should be fundamentally different searches.';
